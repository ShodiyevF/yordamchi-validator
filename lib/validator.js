const validationHelper = require("./validator.helper");

const buildInSharedHelper = validationHelper()
...
function validator(dto, body) {
    try {
        for (const key in dto) {
            if (dto.hasOwnProperty(key)) {
                ...
                if (rules.type === 'object') {
                    if (rules.dto) {
                        const validatorResponse = validator(rules.dto, value);
                ...
                if (rules.type === 'array') {
                    ...
                    if (rules.element_type === 'object') {
                        if (rules.element_dto) { 
                            const checkElementDto = value.find(element => validator(rules.element_dto, element).status !== 200);
                            if (checkElementDto) {
                                return validator(rules.element_dto, checkElementDto)
                            }
                        }
        ...
        return { status: 400, error: 'Internal validation error'};
    }
}
